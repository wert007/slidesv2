...
namespace Slides.SVG
{
	...
	public class IntersectionVertex
	{
		...
		public IntersectionVertex(double index, Vector2 position)
		{
			Index = index;
			Position = position;
			Next = null;
	...
	public static class PathOperationIntersectionHandler
	{
		public static PathOperation[] IntersectionShape(Path a, Path b)
		{
			...
			var aVert = FindVertices(aSeg, bSeg).Where(v => v.Kind != IntersectionKind.None);
			var bVert = FindVertices(bSeg, aSeg).Where(v => v.Kind != IntersectionKind.None);
